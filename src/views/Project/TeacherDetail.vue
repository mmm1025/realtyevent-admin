...
export default {
	...
	methods: {
		loadTeacherData() {
			return this.ajax.get(`/project/teacher?id=${this.id}`).then(res => {
				res = res.data;
				console.log(res);
				if(res.code === 0) {
		...
		/* loadDetail() {
			return this.ajax.get(`/project/teacher/detail/list?id=${this.id}`).then(res => {
				res = res.data;
		...
		/* deleteDetail(id) {
			this.ajax.post(`/project/teacher/detail/delete`, {id : id}).then(res => {
				res = res.data;
		...
		imageUpload(file) {
			...
			this.ajax({
				method : 'post',
				url : '/project/teacher/update/image',
				data : formData,
				headers : {
