<template>
  <div>
		...
		<div id="sliderAddAdmin">
			<h4 class="page-header" v-if="mode === 'add'">슬라이더 추가 (1643x481)</h4>
...
export default {
  props : ['projectId'],
	...
	methods: {
		...
		loadSlideList(){
			this.ajax.get('/project/slider/list',{
        params : {
      ...
      formData.append('project_id', this.projectId);
		...
		sort(){
			...
			this.ajax.post('/project/slider/sort', {list : list}).then(res => {
				// this.loadSlideList();
			})
		},
...
<style>
	#sliderAddAdmin .list {}
	#sliderAddAdmin .page-header button {margin-top:-0.9rem;}

	#sliderAddAdmin .carousel-item {width:100%; height:480px; position:relative; overflow:hidden;}
	#sliderAddAdmin .carousel-item img {width:100%; height:100%; position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); object-fit:cover; z-index: 1;}
	...
	#sliderAddAdmin .row {padding-top:2%;}
	#sliderAddAdmin .row .col-sm-2 {/*padding-right:10px; padding-left:10px; */position:relative;}
	#sliderAddAdmin .row .imgbox {width:100%; height:180px; overflow:hidden; position:relative;}
	#sliderAddAdmin .row .imgbox img {width:100%; height:100%; position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); object-fit:cover; z-index: 1;}
	#sliderAddAdmin .row i {font-size:24px; position:absolute; top:-8px; right:-2px; z-index:99;}
	#sliderAddAdmin .card-body .card-text {position:absolute; top:50%; right:10%; transform:translate(0%,-50%); z-index:100; text-align:right; color:#333;}
	#sliderAddAdmin .card-body .card-text h6 {font-size:18px;}
	#sliderAddAdmin .card-body .card-text h5 {font-size:30px; font-weight:bold;}
	#sliderAddAdmin .card-body .card-text .txt {font-size:14px; width:60%; margin-left:40%;}
