<template>
	<ContentWrapper id="InstructorList">
		<div class="content-heading">
			<div>건설지원센터 관리 > 전문가 목록</div>
		</div>

		<div id="teacherListAdmin">
			<h4 class="page-header d-flex">
				<span>전문가 목록</span>
				<b-btn class="btn btn-dark ml-auto" type="button" variant="primary" v-b-modal.addInstructor>전문가 추가</b-btn>
			</h4>
			<draggable  class="row" v-model="teacherList">
				<div class="col-3" v-for="teacher in teacherList" :key="teacher.id">
					<div class="card card-flat">
						...
						<div class="row no-gutters">							
							...
							<div class="card-body col-sm-8">
								<router-link :to="'/projectTeacher/' + teacher.id">
			...
			<b-modal id="addInstructor" title="전문가 추가" @ok="addTeacher()">
				<form class="form-horizontal" method="get" action="/">
					<fieldset>
						<div class="form-group row">
							<label class="col-md-3 col-form-label">전문가 소속</label>
							<div class="col-md-9">
					...
					<fieldset>
						<div class="form-group row">
							<label class="col-md-3 col-form-label">전문가 이름</label>
							<div class="col-md-9">
					...
					<fieldset>
						<div class="form-group row">
							<label class="col-md-3 col-form-label">전문가 직함</label>
							<div class="col-md-9">
...
<style>
	...
	#teacherListAdmin>.row {margin-top:10px;}
...
<script>
	...
	export default {
		...
		methods : {
			loadTeacherList() {
				this.ajax.get('/project/teacher/list').then(res => {
					res = res.data;
			...
			addTeacher() {
				this.ajax.post('/project/teacher/add', this.teacherData).then(res => {
					res = res.data;
			...
			deleteTeacher(id){
				if(confirm("이 전문가를 삭제하시겠습니까? 영구삭제됩니다.")) {
					this.ajax.post('/project/teacher/delete', {id : id}).then(res => {
						res = res.data;
				...
				this.ajax.post('/project/teacher/update/sort', {
					arr : arr
				})
			},
			updateTeacherShow(teacherId, teacherShow) {
				...
				this.ajax.post('/project/teacher/update', data).then(res => {
					res = res.data;
					if(res.code === 0) {
