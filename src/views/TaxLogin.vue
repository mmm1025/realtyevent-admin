<template>
	<div id="loginWrap">
		<div class="card card-flat">
			...
			<div class="card-body">
				<p class="text-center">땅집고 세무사 로그인 <span v-if="$store.getters.isDevelopment" style="color:red;">(개발)</span></p>
					...
					<button class="btn btn-block btn-green mt-3" type="button" @click="loginBtn">Login</button>
			</div>
		</div>
	</div>
</template>
		...
		data() {
			return {
				login: {
					...
					password: '',
					rememberme: false,
					masterId:'taxsubadmin',
					masterPw:'taxsubadmin',
				}
			}
		},
		methods: {
			...
			loginBtn(){
				this.ajax.post('/tax/login',this.login).then(res => {
					res = res.data;
					if(res.code === 0) {
						localStorage.setItem("token", res.data.token)
						location.href = `/tax/taxReturnResult/${res.data.user.partnerId}`;
					} else {
